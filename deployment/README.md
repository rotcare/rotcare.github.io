...
我们先从一个简单的模型入手，然后逐渐把更真实的现实需求往上加。
...
当然应用与应用之间的所有集成逻辑不可能都用 UI 组件组合来实现。还是会有消息队列的集成，直接的 RPC 调用。这个时候仍然是需要传统的向前兼容，防腐Adapter这些措施的。JSON 作为一种对加字段友好的 RPC 序列化格式仍然会继续流行。再也不可能退回到 SOAP WebService 那种 Schema 版本紧耦合的应用间集成模式去了。
...
* 区分应用内的集成，以及应用之间的集成。应用内选择整体升级，应用之间选择向前兼容，或者防腐Adapter。
* 一个应用的数据切分出多个租户来，从而缩小每次变更的影响范围，更早做全面的集成。只有一个租户升级完没问题之后，才有信心去升级下一个租户。如果因为客户端的平台限制阻碍了这个目标，可以通过解耦 Deployment 和 Release 去绕过。
* 应用与应用之间如果用 UI 组件来集成，就可以避免 RPC 兼容性维护的问题。从而避免向前兼容，防腐Adapter层这些成本。
* 数据库的表结构以及数据迁移逻辑是业务代码的一部分。新建租户也可能是业务逻辑的一部分。